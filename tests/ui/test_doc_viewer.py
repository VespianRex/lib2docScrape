...
from src.ui.doc_viewer import DocViewerApp, VersionInfo, DocumentInfo, DiffRequest, SearchRequest
from src.processors.content.format_handler import DocFormat
...
def test_get_libraries(app):
    ...
    libraries = app._get_libraries()
    assert libraries == ["test-lib", "example-lib"]
    app.version_tracker.get_libraries.assert_called_once()


def test_get_library_versions(app):
    """Test getting library versions."""
    versions = app._get_library_versions("test-lib")
    assert len(versions) == 2
    assert versions[0].version == "1.0.0"
    assert versions[1].version == "1.1.0"
    app.version_tracker.get_versions.assert_called_once_with("test-lib")


def test_get_version_docs(app):
    """Test getting version documents."""
    docs = app._get_version_docs("test-lib", "1.0.0")
    assert len(docs) == 1
    assert docs[0].title == "Test Document"
    app.doc_organizer.get_documents.assert_called_once_with("test-lib", "1.0.0")


def test_get_document(app):
    """Test getting a document."""
    doc = app._get_document("test-lib", "1.0.0", "test/doc.html")
    assert doc is not None
    assert doc["title"] == "Test Document"
    assert doc["content"] == "<h1>Test Document</h1><p>This is a test document.</p>"
    app.doc_organizer.get_document.assert_called_once_with("test-lib", "1.0.0", "test/doc.html")


def test_get_diff(app):
    """Test getting a diff."""
    diff = app._get_diff("test-lib", "test/doc.html", "1.0.0", "1.1.0")
    ...
    app.version_tracker.diff_documents.assert_called_once()


def test_search(app):
    """Test searching."""
    results = app._search("test", library="test-lib")
...
def test_api_libraries(client, app):
...
def test_api_versions(client, app):
