...
class TestURLSecurityExtra:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "https://example.com/docs/../../etc/passwd",
                False,
                "Path traversal up multiple levels",
            ),
            ...
            (
                "file:///etc/passwd",
                True,
                "Allowed scheme: local file access",
            ),  # File scheme is now allowed
            (
                "javascript:void(0)",
                False,
                "Javascript URI scheme",
            ),  # Already covered, but good to re-affirm
            ...
            (
                ...
                False,
                "SSRF attempt to AWS metadata service",
            ),
            (
                "http://metadata.google.internal/computeMetadata/v1/",
                False,
                "SSRF attempt to GCP metadata service",
            ),
            ...
            (
                "http://еxample.com",
                False,
                "Cyrillic 'е' instead of Latin 'e' - Detected as homograph attack",
            ),  # Now properly detected as invalid
            (
                "https://google.com@evil.com/",
                False,
                "URL with username/password attempting to mask domain",
            ),
    ...
    def test_various_security_risks(self, url, expected_valid, description):
        ...
        if not expected_valid:
            ...
            ), f"Error message expected for invalid URL: {description} ({url})"
