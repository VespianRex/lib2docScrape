...
def test_compare_versions(version_manager):
    ...
    diff = version_manager.compare_versions(
        doc_id, "1.0.0", "1.1.0", VersionDiffFormat.UNIFIED
    )
    ...
    diff = version_manager.compare_versions(
        doc_id, "1.0.0", "1.1.0", VersionDiffFormat.CONTEXT
    )
    assert diff is not None
    # Context diff format uses different symbols and spacing
    assert (
        "This is the second line" in diff.diff_content
        and "This is the modified second line" in diff.diff_content
    )

    # Compare versions with HTML diff
    diff = version_manager.compare_versions(
        doc_id, "1.0.0", "1.1.0", VersionDiffFormat.HTML
    )
    assert diff is not None
    assert "<html>" in diff.diff_content
    # HTML diff uses HTML entities like &nbsp; for spaces
    assert "second&nbsp;line" in diff.diff_content and "modified" in diff.diff_content

    # Compare versions with JSON diff
    diff = version_manager.compare_versions(
        doc_id, "1.0.0", "1.1.0", VersionDiffFormat.JSON
    )
    assert diff is not None
    assert "changes" in diff.diff_content

    # Compare identical versions (same content will return existing version)
    version3 = version_manager.add_version(doc_id, content2, "1.1.1")
    # Since content is identical, it returns the existing version 1.1.0
    assert version3.version == "1.1.0"
    # Compare the same version with itself
    diff = version_manager.compare_versions(doc_id, "1.1.0", "1.1.0")
    assert diff is not None
