...
def mock_response():
    ...
    mock.text = AsyncMock(return_value="<html><head><title>Test</title></head><body>Test content</body></html>")
    return mock
...
async def test_crawl_valid_url(backend, mock_session):
    ...
    with patch.object(backend, '_ensure_session', AsyncMock()) as mock_ensure_session:
...
async def test_circuit_breaker(backend):
    ...
    with patch.object(backend, '_fetch_with_retry', AsyncMock(return_value=CrawlResult(
...
async def test_validate_content(backend):
    ...
    valid_result.content = {"html": "<html><head><title>Test</title></head><body>Test content</body></html>"}
...
async def test_process_content(backend):
    ...
    # Create a valid result
    valid_result = MagicMock()
    valid_result.content = {"html": "<html><head><title>Test</title></head><body><h1>Test</h1><p>Content</p></body></html>"}
    valid_result.url = "https://example.com"
    ...
    with patch.object(backend.content_processor, 'process', AsyncMock(return_value=mock_processed_content)):
        # Process the content
