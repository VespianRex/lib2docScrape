import pytest
from unittest.mock import AsyncMock, MagicMock # Added MagicMock back for the spec in one test
...
from tests.fixtures.scrapy_fixtures import mock_aiohttp_session # Import our fixture
...
async def test_scrapy_backend_crawl_success(mocker: MockerFixture):
    ...
    mock_response.text = AsyncMock(return_value=html_content) # .text() is an async method
    ...
    mocker.patch('src.backends.scrapy_backend.aiohttp.ClientSession', return_value=mock_session_instance)
    ...
    result: CrawlResult = await backend.crawl(url_info, crawler_config) # Changed BackendCrawlResult to CrawlResult
...
async def test_scrapy_backend_crawl_http_error(mocker: MockerFixture):
    ...
    assert result.error is None # ScrapyBackend currently sets error to None if response is received
...
async def test_scrapy_backend_crawl_exception(mocker: MockerFixture):
    ...
    mock_session_instance.get = MagicMock(side_effect=Exception("Network error")) # Corrected
...
async def test_scrapy_backend_crawl_invalid_url(mocker: MockerFixture):
    ...
    result: CrawlResult = await backend.crawl(mock_url_info, crawler_config) # Changed BackendCrawlResult to CrawlResult
    ...
    assert "Invalid URL provided: ftp://invalid" in result.error or "Invalid URL provided" in result.error
...
async def test_rate_limiting(mocker: MockerFixture, mock_aiohttp_session): 
    ...
    mocker.patch('src.backends.scrapy_backend.aiohttp.ClientSession', return_value=mock_session)
...
@pytest.mark.asyncio
async def test_different_content_types(mocker: MockerFixture, mock_aiohttp_session): # Use our fixture
