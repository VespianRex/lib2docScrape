...
class TestBackendPerformanceTracker:
    ...
    async def test_persistence_load_save(self, temp_storage_path):
        ...
        domain = "example.com"
        await tracker1.record_performance(
            "http",
            domain,
            {
                "response_time": 1.0,
    ...
    async def test_performance_data_cleanup(self, temp_storage_path):
        ...
        tracker = BackendPerformanceTracker(
            storage_path=temp_storage_path, max_history_days=7
        )
    ...
    async def test_real_time_performance_monitoring(
        self, temp_storage_path, mock_backend
    ):
        ...
        with patch("src.performance.backend_tracker.PSUTIL_AVAILABLE", True), patch(
            "psutil.Process"
            ...
            # Start monitoring
            domain = "test.com"
            monitoring_context = await tracker.start_monitoring(
                mock_backend.name, domain
            )
