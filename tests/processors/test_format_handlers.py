...
async def test_html_handler_process(html_handler, html_processor):
    ...
    result = await html_handler.process("<html><body>Test</body></html>", "https://example.com")

    # Check that the HTML processor was called
    html_processor.process.assert_called_once_with("<html><body>Test</body></html>", "https://example.com")

    # Check the result
    assert result["formatted_content"] == "<html><body>Test</body></html>"
    assert result["structure"] == [{"type": "heading", "level": 1, "title": "Test"}]
    assert result["headings"] == [{"level": 1, "text": "Test", "id": "", "type": "heading"}]
...
async def test_markdown_handler_process(markdown_handler):
    ...
    with patch("markdown.markdown", return_value="<h1>Heading</h1><p>Paragraph</p>"):
        ...
        with patch("src.processors.content_processor.ContentProcessor") as mock_processor_class:
            ...
            result = await markdown_handler.process("# Heading\n\nParagraph", "https://example.com")

            # Check the result
            assert result["formatted_content"] == "# Heading\n\nParagraph"
            assert result["structure"] == [{"type": "heading", "level": 1, "title": "Heading"}]
...
def test_asciidoc_handler_can_handle(asciidoc_handler):
    ...
    assert asciidoc_handler.can_handle("[source,python]\n----\nprint('Hello')\n----") is True
...
def test_format_detector_detect_format(format_detector):
    ...
    assert format_detector.detect_format("", "text/markdown").get_format_name() == "Markdown"
    assert format_detector.detect_format("", "text/x-rst").get_format_name() == "reStructuredText"
    assert format_detector.detect_format("", "text/asciidoc").get_format_name() == "AsciiDoc"

    # Test with content
    assert format_detector.detect_format("<html><body>Test</body></html>").get_format_name() == "HTML"
    assert format_detector.detect_format("# Heading\n\nParagraph").get_format_name() == "Markdown"
    # Use a more specific RST pattern that won't be confused with Markdown
    assert format_detector.detect_format(".. directive:: value").get_format_name() == "reStructuredText"
    assert format_detector.detect_format("= Document Title").get_format_name() == "AsciiDoc"
...
def test_content_type_detector():
    ...
    assert ContentTypeDetector.detect_from_content("<!DOCTYPE html><html><body>Test</body></html>") == "text/html"
    assert ContentTypeDetector.detect_from_content("# Heading\n\nParagraph") == "text/markdown"
    assert ContentTypeDetector.detect_from_content("=======\nHeading\n=======\n\nParagraph") == "text/x-rst"
    assert ContentTypeDetector.detect_from_content("= Document Title") == "text/asciidoc"
