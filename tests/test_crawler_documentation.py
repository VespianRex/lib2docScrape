...
class TestDocumentationCrawler:
    ...
    def test_documentation_crawler_init_defaults(self):
        ...
        assert crawler.config.requests_per_second == 5.0
        assert crawler.config.max_retries == 3
        assert crawler.config.user_agent == "Python Documentation Scraper/1.0"
    ...
    def test_should_crawl_url(self):
        """Test 7.1: Test _should_crawl_url method with various URL types."""
        crawler = DocumentationCrawler()
        
        # Create a basic target
        target = crawler.config or CrawlerConfig()
