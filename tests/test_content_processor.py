...
class TestBasicFunctionality:
    ...
    async def test_extract_text_with_nested_content(
        self, processor
    ):  # Added processor fixture
    ...
    async def test_extract_text_with_special_characters(
        self, processor
    ):  # Added processor fixture
        ...
        result = await processor.process(html)
        assert (
            "Special characters: & < > \" '" in result.content["formatted_content"]
        )  # Markdownify unescapes

    async def test_extract_text_with_complex_content(
        self, processor
    ):  # Added processor fixture
    ...
    async def test_extract_text_with_mixed_content_types(
        self, processor
    ):  # Added processor fixture
...
class TestCodeBlockExtraction:
    ...
    @pytest.mark.parametrize(
        "language,code,expected_html_class",
        [
    ...
    async def test_extract_code_blocks(
        self, processor, language, code, expected_html_class
    ):
        if language:
...
class TestContentStructureExtraction:
    ...
    async def test_extract_content_structure(self, processor):
        ...
        assert (
            len(structure) == 6
        )  # Based on previous understanding of structure handler
    ...
    async def test_extract_headings_hierarchy(self, processor):
        ...
        assert not any(
            h["text"] == "Fourth Level" for h in headings
        )  # Due to structure_handler config
...
class TestPrimaryMarkdownOutput:
    ...
    async def test_format_as_markdown(
        self, processor
    ):  # Renamed from test_format_as_markdown_markdownify
        html = "<html><body><h1>Title</h1><p>Paragraph.</p></body></html>"
        result = await processor.process(html)
        expected_markdown = "# Title\n\nParagraph."
        assert result.content["formatted_content"].strip() == expected_markdown
    ...
    @pytest.mark.parametrize(
        "language,code,expected_html_class,expected_md_content",
        [
            (
                "javascript",
    ...
    async def test_extract_code_blocks(
        self, processor, language, code, expected_html_class, expected_md_content
    ):  # Renamed
        if language:
            html = f'<pre><code class="{expected_html_class}">{code}</code></pre>'
        else:
