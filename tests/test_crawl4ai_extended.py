...
async def test_crawl_with_urlinfo(crawl4ai_backend):
    ...
    async with crawl4ai_backend as backend:
        result = await backend.crawl(url_info)
    
    assert result is not None
    assert result.url == url_info.normalized
...
async def test_rate_limiting_precision(crawl4ai_backend):
    ...
    async with crawl4ai_backend as backend:
        results = await asyncio.gather(*[
            backend.crawl(url) for url in urls
        ])
...
async def test_error_propagation(crawl4ai_backend):
    ...
    error_cases = [
        ("https://nonexistent.example.com", 500),  # DNS error
        ("https://example.com/notfound", 404),     # Not found
        ("https://example.com/error", 500),        # Server error
    ]
...
@pytest.mark.asyncio
async def test_retry_behavior(crawl4ai_backend):
    """Test retry behavior with failing requests."""
    url = "https://example.com/unstable"
    attempts = []
    
    # Create a session that fails twice then succeeds
    ...
    crawl4ai_backend._session.get = mock_get
    async with crawl4ai_backend as backend:
        result = await backend.crawl(url)
