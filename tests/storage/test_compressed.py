...
from src.storage.compressed.storage import CompressedStorage, CompressionConfig, CompressionFormat
...
def test_save_load_json(storage, temp_dir):
    ...
    compressed_path = f"{file_path}.{storage.config.format}"
...
def test_different_compression_formats(temp_dir):
    ...
    for format in CompressionFormat:
        ...
        # Save data
        file_path = os.path.join(temp_dir, f"test_{format}.json")
        storage.save_json(data, file_path)
