...
class TestBackendSelectorIntegration:
    ...
    def mock_backend(self):
        ...
        backend.crawl = AsyncMock(return_value=BackendCrawlResult(
            url="https://test.com",
    ...
    async def test_backend_selector_with_performance_tracking(
        ...
        backend_selector.register_backend("test_backend", mock_backend, default_criteria)

        # Record some performance data
        domain = "test.com"
        await performance_tracker.record_performance("test_backend", domain, {
            "response_time": 1.0,
            "memory_usage": 50.0,
            "cpu_usage": 15.0,
            "success": True,
    ...
    async def test_performance_based_backend_selection(
        ...
        fast_backend.crawl = AsyncMock(return_value=BackendCrawlResult(
            url="https://test.com", content={"html": "content"}, metadata={}, status=200
        ))

        slow_backend = MagicMock(spec=CrawlerBackend)
        slow_backend.name = "slow_backend"
        slow_backend.crawl = AsyncMock(return_value=BackendCrawlResult(
            url="https://test.com", content={"html": "content"}, metadata={}, status=200
        ))

        # Register backends with criteria
        backend_selector.register_backend("fast_backend", fast_backend, default_criteria)
        backend_selector.register_backend("slow_backend", slow_backend, default_criteria)
        
        domain = "test.com"
        
        # Record performance data - fast backend performs better
        await performance_tracker.record_performance("fast_backend", domain, {
            "response_time": 0.5,
            "memory_usage": 30.0,
            "cpu_usage": 10.0,
            "success": True,
        ...
        await performance_tracker.record_performance("slow_backend", domain, {
            "response_time": 3.0,
            "memory_usage": 150.0,
            "cpu_usage": 50.0,
            "success": True,
    ...
    async def test_performance_tracking_during_crawl(
        ...
        with patch.object(performance_tracker, 'start_monitoring') as mock_start, \
             patch.object(performance_tracker, 'stop_monitoring') as mock_stop, \
             patch.object(performance_tracker, 'record_performance') as mock_record:
    ...
    async def test_performance_data_persistence(
        self, backend_selector, performance_tracker, mock_backend, temp_storage_path, default_criteria
    ):
    ...
    async def test_adaptive_backend_switching(
        ...
        await performance_tracker.record_performance("backend_a", domain, {
            "response_time": 1.0,
            "memory_usage": 40.0,
            "cpu_usage": 10.0,
            "success": True,
        ...
        await performance_tracker.record_performance("backend_b", domain, {
            "response_time": 2.0,
            "memory_usage": 80.0,
            "cpu_usage": 25.0,
            "success": True,
    ...
    async def test_error_handling_in_performance_tracking(
        ...
        with patch.object(performance_tracker, 'start_monitoring', side_effect=Exception("Monitoring failed")):
