...
from src.backends.lightpanda_backend import LightpandaBackend
...
async def test_content_processor_performance():
    ...
    print(f"Content processor performance:")
...
async def test_backend_concurrency():
    ...
    # Log performance metrics
    print(f"Backend concurrency performance:")
    ...
    assert all(result.status == 200 for result in concurrent_results)
...
async def test_benchmark_performance():
    ...
    # Register backends
    with patch("src.backends.crawl4ai_backend.Crawl4AIBackend") as mock_crawl4ai, \
        ...
        mock_crawl4ai_instance.crawl = AsyncMock(return_value=MagicMock(status=200, content={"html": SAMPLE_HTML}))
        ...
        mock_lightpanda_instance.crawl = AsyncMock(return_value=MagicMock(status=200, content={"html": SAMPLE_HTML}))
        ...
        print(f"  Average time per benchmark: {benchmarking_time / (len(urls) * len(benchmark.backends)):.2f} seconds")
...
async def test_incremental_crawling():
    ...
    backend = AsyncMock()
    backend.crawl = AsyncMock(return_value=MagicMock(status=200, content={"html": SAMPLE_HTML}))
    ...
    for url_info in urls:
        result = await backend.crawl(url_info)
        processed = await processor.process(result.content.get("html", ""), url_info.normalized_url)
    ...
    organizer.get_document_by_url = MagicMock(return_value=MagicMock())  # All documents exist
    ...
    # Log performance metrics
    print(f"Incremental crawling performance:")
