...
class TestURLNormalizationIDN:
    ...
    @pytest.mark.parametrize(
        "input_url, expected_normalized_url, description",
        [
            # Basic IDN
            (
                "https://例子.com",
            ...
            (
                ...
                "Chinese IDN with path and query",
            ),
            # Mixed ASCII and IDN
            (
            ...
            (
                ...
                "IDN in subdomain part",
            ),  # Removed trailing slash
            # Punycode input (should remain punycode)
            (
                "https://xn--fsqu00a.com",
                "https://xn--fsqu00a.com",
                "Punycode input should not change",
            ),  # Removed trailing slash
    ...
    def test_idn_normalization(self, input_url, expected_normalized_url, description):
        ...
        ), f"Failed check: {description} ({input_url})"
        assert (
            info.is_valid
        ), f"URL should be valid after normalization: {description} ({input_url})"
