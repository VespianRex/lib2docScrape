...
def processed_content():
    ...
    content.content = {"formatted_content": "# Test Function\n\nThis is a test function."}
    content.structure = [{"type": "heading", "level": 1, "title": "Test Function"}]
    content.headings = [{"level": 1, "text": "Test Function", "id": "", "type": "heading"}]
...
def test_registry_register_version(registry):
    ...
    assert library["versions"]["3.0.0"]["documentation_url"] == "https://docs.test-lib.com/v3.0.0"
...
def test_registry_verify_doc_site(registry):
    ...
    is_valid, name, version = registry.verify_doc_site("https://docs.test-lib.com/v1.0.0/api")
...
def test_registry_save_load():
    ...
    try:
        ...
        assert loaded_registry.libraries["test_lib"]["base_url"] == "https://docs.test-lib.com"
    finally:
        # Clean up
...
def test_tracker_add_documentation(tracker, processed_content):
    ...
    assert tracker.version_docs["test_lib"]["1.0.0"][doc_id]["url"] == processed_content.url
    assert tracker.version_docs["test_lib"]["1.0.0"][doc_id]["title"] == processed_content.title
...
def test_tracker_compare_versions(tracker, processed_content):
    ...
    modified_content.content = {"formatted_content": "# Test Function\n\nThis is a modified test function."}
...
@pytest.mark.skipif(not hasattr(LibraryVersionTracker, "generate_visual_diff"), reason="Visual diff not implemented")
def test_tracker_generate_visual_diff(tracker, processed_content):
    ...
    with patch("src.organizers.library_version_tracker.HAS_VISUALIZATION", True), \
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            output_file = tracker.generate_visual_diff("test_lib", "1.0.0", "2.0.0", temp_dir)
