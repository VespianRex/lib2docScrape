...
class TestURLParsingWindows:
    ...
    @pytest.mark.parametrize(
        "input_url, base_url, expected_normalized_url, description",
        [
            ...
            ),  # Expect invalid or needs base
            # UNC path resolved relative to base - this is valid behavior
            (
                "\\\\server\\share\\file.txt",
                "https://example.com/docs/",
                "https://example.com/docs/server/share/file.txt",
                "UNC path resolved relative to base",
            ),
            ...
            (
                ...
                None,
                "file:///C:/Users/User/Documents/file.html",
                "File scheme with absolute Windows path - now allowed",
            ),
            ...
            (
                ...
                None,
                "Relative path traversal with backslashes (should be invalid)",
            ),  # Expect invalid due to traversal check
        ],
    )
    def test_windows_path_handling(
        self, input_url, base_url, expected_normalized_url, description
    ):
        ...
        if (
            "(should be invalid)" in description
        ):  # Check description for expected invalidity
            assert (
                not info.is_valid
            ), f"URL should be invalid: {description} ({input_url})"
            assert (
                info.error_message is not None
                and "Path traversal attempt detected" in info.error_message
            ), f"Incorrect error for {input_url}: {info.error_message}"
            ...
            assert (
                not info.is_valid or info.url_type == URLType.UNKNOWN
            ), f"Expected invalid or unknown: {description} ({input_url})"
        else:
            ...
            assert (
                info.normalized_url == expected_normalized_url
            ), f"Normalization failed: {description} ({input_url})"
    ...
    def test_windows_drive_as_scheme_or_host(self):
        ...
        assert (
            info.error_message is not None and "Invalid scheme" in info.error_message
        ), "Error message should indicate invalid scheme"
