...
def test_security_config_constants():
    ...
    assert (
        "file" not in URLSecurityConfig.DISALLOWED_SCHEMES
    )  # Changed: file is no longer disallowed
    assert "ftp" in URLSecurityConfig.DISALLOWED_SCHEMES
    assert "ftps" in URLSecurityConfig.DISALLOWED_SCHEMES

    # Test length limits
    assert (
        URLSecurityConfig.MAX_PATH_LENGTH == 2048
    )  # Updated to match the corrected value in security.py
    assert (
        URLSecurityConfig.MAX_QUERY_LENGTH == 2048
    )  # Corrected based on src/utils/url/security.py
...
def test_security_regex_patterns():
    ...
    assert (
        URLSecurityConfig.INVALID_CHARS.search("abcdef") is None
    )  # Correct assertion for no match
    ...
    assert URLSecurityConfig.NULL_BYTE_PATTERN.search(
        "file\x00.jpg"
    )  # Test against actual null byte
