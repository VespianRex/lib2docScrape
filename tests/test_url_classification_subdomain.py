...
class TestURLClassificationSubdomain:
    ...
    @pytest.mark.parametrize(
        "url, base_url, expected_type, description",
        [
            ...
            (
                ...
                "https://docs.example.com/",
                URLType.INTERNAL,
                "Different subdomain, same parent domain - treated as internal",
            ),
            (
                "//blog.example.com/latest",
                "https://docs.example.com/",
                URLType.INTERNAL,
                "Protocol-relative link to different subdomain",
            ),
            ...
            (
                ...
                "https://docs.example.com/",
                URLType.INTERNAL,
                "Link from subdomain to parent domain",
            ),
            ...
            (
                ...
                "https://example.com/",
                URLType.INTERNAL,
                "Link from parent to known subdomain - treated as internal",
            ),  # Assuming subdomains are considered internal to the parent
            # Edge case: www vs non-www (depends on strictness, often treated as same logical site but technically different hosts)
            (
                "https://www.example.com",
                "https://example.com/",
                URLType.INTERNAL,
                "www vs non-www, same domain - often internal",
            ),  # Behavior might vary based on config
            (
                "https://example.com",
                "https://www.example.com/",
                URLType.INTERNAL,
                "non-www vs www, same domain - often internal",
            ),  # Behavior might vary based on config
            # Deeply nested subdomains
            (
                "https://api.internal.dev.example.com",
                "https://docs.example.com/",
                URLType.INTERNAL,
                "Deeply nested different subdomain",
            ),
            (
                "https://api.internal.dev.example.com/status",
                "https://api.internal.dev.example.com/",
                URLType.INTERNAL,
                "Internal link within deeply nested subdomain",
            ),
            # Relative link potentially crossing subdomain boundary (should resolve within base_url's domain)
            (
                "../marketing/campaign",
                "https://docs.example.com/team/",
                URLType.INTERNAL,
                "Relative path resolving within the base domain",
            ),  # Resolves to docs.example.com/marketing/campaign
        ],
    )
    def test_subdomain_classification(self, url, base_url, expected_type, description):
        ...
        assert (
            info.url_type == expected_type
        ), f"Classification failed: {description} ({url} relative to {base_url})"
