...
async def test_special_characters():
    ...
    content = (
        "<div><p>Test content with entities: &amp; &lt; &gt; &quot; &apos;</p></div>"
    )
    result = await processor.process(content, "https://example.com")
...
async def test_large_content():
    ...
    # Mock time.time() with enough values for all logging calls
    # The logging system may call time.time() multiple times during processing
    with patch("time.time") as mock_time:
        # Provide a repeating sequence of time values
        mock_time.side_effect = [
            ...
            9,
            10,
        ] * 10  # 100 values should be enough
