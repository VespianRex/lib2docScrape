...
try:
    ...
    logging.error("Could not import ProcessedContent from src.processors.content_processor. Ensure 'src' is in PYTHONPATH or installed correctly.")
...
def to_processed_content(
    ...
    if isinstance(content_input, dict):
        ...
        raise TypeError("Input must be a ProcessedContent, DocumentContent object, or a dictionary.")
        ...
        for term in terms:
            for idx_term in [term, term.lower()]:
                if idx_term not in self.search_indices:
                    ...
                    self.search_indices[idx_term] = SearchIndex(term=idx_term, documents=[], context={})
                if not any(d[0] == doc_id for d in self.search_indices[idx_term].documents):
                    self.search_indices[idx_term].documents.append((doc_id, 1.0))

    def search(self, query: str, documents: Dict[str, Any]) -> List[Tuple[str, float, List[str]]]:
        """
        ...
        matches: Dict[str, Dict[str, Any]] = defaultdict(lambda: {"score": 0, "reasons": []})

        for doc_id, doc in documents.items():
            # Cache tokenized title and text tokens
            title_tokens: Set[str] = set(self.text_processor.tokenize(doc.metadata.title))
            ...
            doc_index_terms: Set[str] = {
                term for term, index_entry in self.search_indices.items() if any(d[0] == doc_id for d in index_entry.documents)
            }
        ...
        results: List[Tuple[str, float, List[str]]] = [
            (doc_id, data["score"], data["reasons"]) for doc_id, data in matches.items() if data["score"] > 0
        ]
...
class DocumentMetadata(BaseModel):
    ...
    def add_version(self, processed_content_dict: Dict[str, Any], max_versions: Optional[int] = None) -> None:
        ...
        try:
            hashable_dict: Dict[str, Any] = {k: v for k, v in processed_content_dict.items() if k not in ['errors']}
...
class Document:
    ...
    """
    def __init__(self, metadata: Optional[DocumentMetadata] = None, content: Optional[Union[Dict[str, Any], Any]] = None) -> None:
...
class DocumentOrganizer:
    ...
    def _categorize_document(self, content: ProcessedContent) -> Optional[str]:
        ...
        """
        text: str = str(content.content.get("text", "")).lower() if content.content else ""
        title: str = content.title.lower()
        url_path: str = urlparse(content.url).path.lower() if content.url else ""
        path_segments: Set[str] = {segment for segment in url_path.split('/') if segment}
        for category, patterns in self.config.category_rules.items():
            for pattern in patterns:
                pattern_lower: str = pattern.lower()
                if (pattern_lower in title or
                    (text and pattern_lower in text) or
                    pattern_lower in path_segments):
                    return category
        return None
    ...
    def _extract_references(self, content: ProcessedContent) -> Dict[str, List[str]]:
        ...
        if content.content and "links" in content.content:
            for link in content.content["links"]:
                ...
                if not url:
                    continue
                if link_type == "internal" or (not link_type and urlparse(url).netloc == urlparse(content.url).netloc):
    ...
    def _generate_index_terms(self, content: ProcessedContent) -> List[str]:
        ...
        if content.headings:
            for heading in content.headings:
                if "text" in heading:
                    heading_words: List[str] = self.text_processor.extract_terms(heading["text"])
        ...
        if content.url:
            ...
            path_segments: List[str] = [segment for segment in path.split("/") if segment]
    ...
    def _update_search_indices(self, doc_id: str, content: ProcessedContent) -> None:
        ...
        for term in terms:
            ...
            if term_lower not in self.search_indices:
                self.search_indices[term_lower] = SearchIndex(term=term_lower, documents=[], context={})
            if not any(d[0] == doc_id for d in self.search_indices[term_lower].documents):
                self.search_indices[term_lower].documents.append((doc_id, 1.0))
            if doc_id not in self.search_indices[term_lower].context:
                text_content: str = content.content.get('text', '') if content.content else ''
                snippet: str = (text_content[:100] + "...") if len(text_content) > 100 else text_content
                self.search_indices[term_lower].context[doc_id] = [snippet] if snippet else []

    def add_document(self, content_input: Union[ProcessedContent, DocumentContent, dict]) -> str:
        ...
        try:
            content_dict: Dict[str, Any] = asdict(processed_content) if hasattr(processed_content, '__dataclass_fields__') or isinstance(processed_content, BaseModel) else processed_content.__dict__
        except TypeError:
        ...
        if existing_doc_id:
            ...
            doc_meta.add_version(content_dict, max_versions=self.config.max_versions_to_keep)
    ...
    def search(self, query: str, category: Optional[str] = None) -> List[Tuple[str, float, List[str]]]:
        ...
        results_map: Dict[str, Dict[str, Any]] = defaultdict(lambda: {"score": 0.0, "matches": []})
        ...
        for doc_id, terms in doc_terms_map.items():
            ...
            if category is not None:
                ...
                if doc_cat != category and not (category == "guide" and doc_cat is None):
                    continue
            common_terms: Set[str] = query_terms.intersection(terms)
            if common_terms:
                results_map[doc_id]["score"] += len(common_terms)
                for term in common_terms:
                    snippet_list: List[str] = self.search_indices.get(term, SearchIndex(term=term, documents=[], context={})).context.get(doc_id, [])
        ...
        final_results: List[Tuple[str, float, List[str]]] = [
            (doc_id, data["score"], data["matches"]) for doc_id, data in results_map.items() if data["score"] > 0
        ]
    ...
    def create_collection(self, name: str, description: str, doc_ids: List[str]) -> str:
        ...
        Returns:
            Collection ID
        """
        valid_ids: List[str] = [doc_id for doc_id in doc_ids if doc_id in self.documents]
    ...
    def get_related_documents(self, doc_id: str) -> List[Tuple[str, float]]:
        ...
        for other_id, shared_term_count in related_scores.items():
            if other_id in self.documents:
                ...
                if similarity >= self.similarity_threshold or (doc_id in self.documents and other_id in self.documents):
...
def create_test_content(url: str, title: str, content: Dict[str, Any]) -> DocumentContent:
    """
