...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cov", action="store_true", help="Run tests with coverage report"
    )
    ...
    parser.add_argument(
        "--no-warnings", action="store_true", help="Disable warning capture"
    )
...
def get_test_files(
    unit_only: bool = False, integration_only: bool = False
) -> list[str]:
    """
...
def run_tests(files: list[str], args: argparse.Namespace) -> tuple[int, Optional[str]]:
    """
...
def print_summary(
    start_time: float, exit_code: int, cov_report_path: Optional[str]
) -> None:
    """
    ...
    if cov_report_path:
        ...
        if os.path.exists(cov_report_path):
            print(
                f"Open {cov_report_path}/index.html in your browser to view the report"
            )
...
def main() -> None:
    ...
    # Get test files
    files = get_test_files(unit_only=args.unit, integration_only=args.integration)
