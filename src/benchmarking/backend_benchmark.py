...
class BackendBenchmark:
    ...
    async def benchmark_urls(self, urls: List[str], config=None) -> Dict[str, List[BenchmarkResult]]:
        """
        ...
        """
        all_results: Dict[str, List[BenchmarkResult]] = {name: [] for name in self.backends}
    ...
    def generate_report(self, output_file: Optional[str] = None) -> str:
        ...
        for result in self.results:
            data.append({
                ...
                "Memory Usage (MB)": result.memory_usage if result.memory_usage else 0,
        ...
        report += "## Success Rate\n\n"
        success_rate = df.groupby("Backend")["Success"].mean().sort_values(ascending=False)
    ...
    def _generate_charts(self, df: pd.DataFrame, base_filename: str) -> None:
        ...
        plt.figure(figsize=(10, 6))
        content_size = df.groupby("Backend")["Content Size (bytes)"].mean().sort_values()
