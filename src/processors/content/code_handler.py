...
class CodeHandler:
        ...
        for pre in soup.find_all("pre"):
            ...
            if code:
                ...
                if language and self.is_language_supported(language):
                    ...
                    language
                ):  # Language detected but not supported, remove the pre block
                    pre.decompose()
                else:  # No language detected, format as plain code block
                    markdown_block = self._format_code_block(
                        code, None
                    )  # Pass None for language
    ...
    def _detect_language(self, code: Tag) -> Optional[str]:
        ...
        if "class" in code.attrs:
            classes = (
                code["class"] if isinstance(code["class"], list) else [code["class"]]
            )
            for cls in classes:
                if cls.lower() == "python":
                    return "python"
