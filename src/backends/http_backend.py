...
# Import CrawlerConfig from crawler conditionally for type hinting
if TYPE_CHECKING:
    from ..crawler import CrawlerConfig
from ..utils.url import URLInfo  # Import URLInfo
    ...
    async def crawl(
        ...
        try:
            if not self.session:
                ...
                if (
                    config
                    and config.user_agent
                    and headers_to_use.get("User-Agent") != config.user_agent
                ):
            ...
            async with self.session.get(
                url_to_fetch,
                ssl=None
                if not current_config.verify_ssl
                else True,  # Use current_config
            ...
            logging.error(
                f"HTTP error fetching {url_to_fetch}: Status {e.status}, Message: {e.message}"
            )
            return CrawlResult(
                url=url_to_fetch,
                content={},
