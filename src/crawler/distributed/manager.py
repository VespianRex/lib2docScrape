...
from .models import (
    ...
    DistributedConfig, TaskStatus, ManagerStatus
)
...
class DistributedCrawlManager:
    ...
    def __init__(self, config: Optional[DistributedConfig] = None):
        ...
        logger.info(f"Distributed crawl manager initialized with max_workers={self.config.max_workers}")
    ...
    async def register_worker(self, worker_info: WorkerInfo) -> None:
        ...
        if worker_id in self.workers:
            logger.warning(f"Worker {worker_id} already registered, updating information")
        else:
    ...
    def add_task(self, task: WorkerTask) -> None:
        ...
        if len(self.task_queue) >= self.config.task_queue_limit:
            raise ValueError(f"Task queue is full (limit: {self.config.task_queue_limit})")
    ...
    async def check_worker_timeouts(self) -> None:
        ...
        for worker_id, worker_info in list(self.workers.items()):
            ...
            if seconds_since_heartbeat > timeout_threshold:
                logger.warning(f"Worker {worker_id} timed out (last heartbeat: {seconds_since_heartbeat:.1f}s ago)")
