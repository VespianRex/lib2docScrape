...
from .models import TaskResult, WorkerHeartbeat, WorkerInfo, WorkerStatus, WorkerTask
...
class CrawlWorker:
    ...
    def __init__(
        ...
        heartbeat_callback: Optional[
            Callable[[WorkerHeartbeat], Awaitable[None]]
        ] = None,
    ):
        """
        ...
        logger.info(
            f"Worker {self.worker_id} initialized on {self.hostname} ({self.ip_address})"
        )
    ...
    async def process_task(self, task: WorkerTask) -> TaskResult:
        ...
        logger.info(
            f"Worker {self.worker_id} processing task {task.task_id} for URL {task.url}"
        )

        # Create result
        result = TaskResult(
            task_id=task.task_id, worker_id=self.worker_id, url=task.url, success=False
        )
        ...
        try:
            ...
            # Check if crawl_results is a list or a single result
            ...
            result.links_found = sum(
                len(getattr(r, "links", [])) for r in crawl_results
            )
            result.content_type = (
                crawl_results[0].content_type if crawl_results else None
            )
            ...
            # Store result data
            result.result_data = {"results": [r.model_dump() for r in crawl_results]}
