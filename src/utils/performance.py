...
) -> Union[dict[str, PerformanceMetrics], Optional[PerformanceMetrics]]:
    """
...
def _update_metrics(
    name: str, elapsed: float, error: bool = False, cache_hit: bool = False
) -> None:
    """
...
def memoize(
    func: Optional[F] = None, *, config: Optional[CacheConfig] = None
) -> Callable[[F], F]:
    """
        ...
        if cache_config.strategy == CacheStrategy.LRU:
            ...
            def wrapper(*args: Any, **kwargs: Any) -> Any:
                ...
                key_kwargs = {
                    ...
                    if k not in cache_config.ignore_kwargs
                }
...
def rate_limit(
    func: Optional[F] = None, *, config: Optional[RateLimitConfig] = None
) -> Callable[[F], F]:
    """
        ...
        async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            async with lock:
                ...
                if tokens < 1:
                    ...
                    logger.debug(
                        f"Rate limit exceeded for {func.__name__}, waiting {wait_time:.2f}s"
                    )
                    ...
                    elapsed = now - last_refill
                    new_tokens = int(
                        elapsed * rate_config.max_calls / rate_config.period
                    )
...
def throttle(
    func: Optional[F] = None, *, config: Optional[ThrottleConfig] = None
) -> Callable[[F], F]:
    """
    ...
    def decorator(func: F) -> F:
                ...
                try:
                    if asyncio.iscoroutinefunction(func):
                        result = await asyncio.wait_for(
                            func(*args, **kwargs), timeout=throttle_config.timeout
                        )
                    else:
                    ...
                    _update_metrics(func.__name__, elapsed, error=True)
                    raise TimeoutError(
                        f"Function {func.__name__} timed out after {throttle_config.timeout}s"
                    ) from e
                except Exception:
                    elapsed = time.time() - start_time
                    _update_metrics(func.__name__, elapsed, error=True)
        ...
        def sync_wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            with ThreadPoolExecutor(
                max_workers=throttle_config.max_concurrency
            ) as executor:
                start_time = time.time()
                try:
