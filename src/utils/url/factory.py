...
def create_url_info(url: Optional[str], base_url: Optional[str] = None) -> URLInfo:
    ...
    try:
        ...
        if URLSecurityConfig.CONTROL_CHARS_PATTERN.search(url_info._raw_url):
            ...
            url_info._initialized = True
            logger.debug(
                f"URLInfo Invalid (Control Chars): raw_url='{url_info._raw_url}', error='{url_info.error_message}'"
            )
        ...
        try:
            ...
            if URLSecurityConfig.NULL_BYTE_PATTERN.search(
                decoded_path
            ) or URLSecurityConfig.NULL_BYTE_PATTERN.search(decoded_query):
                url_info.is_valid = False
                url_info.error_message = "Null byte detected in raw URL path or query"
                url_info._normalized_url = url_info._raw_url  # Fallback
                url_info._initialized = True
                logger.debug(
                    f"URLInfo Invalid (Null Byte Raw): raw_url='{url_info._raw_url}', error='{url_info.error_message}'"
                )
                return url_info  # Exit initialization early
        except Exception as decode_err:
            url_info.is_valid = False
            url_info.error_message = (
                f"URL decoding failed during initial security check: {decode_err}"
            )
            url_info._normalized_url = url_info._raw_url  # Fallback
            url_info._initialized = True
            logger.warning(
                f"URLInfo Initial Security Check Decode Error: raw_url='{url_info._raw_url}', error='{url_info.error_message}'"
            )
            return url_info
        ...
        url_info._initialized = True
        logger.warning(
            f"URLInfo Initial Security Check Error: raw_url='{url_info._raw_url}', error='{url_info.error_message}'"
        )
        return url_info
    ...
    try:
        logger.debug(
            f"URLInfo Init (Post-Security): raw_url='{url_info._raw_url}', base_url='{url_info.base_url}'"
        )
        # --- Parsing and Resolution --- Resolve WITH fragment first
        try:
            resolved_url_str_with_frag = resolve_url(
                url_info._raw_url, url_info.base_url, keep_fragment=True
            )
            ...
            logger.warning(
                f"URLInfo Resolution Error: raw_url='{url_info._raw_url}', error='{url_info.error_message}'"
            )
            # Set initialized before returning to avoid issues in finally block
            url_info._initialized = True
            return url_info  # Exit initialization early

        # Store fragment before removing it
        # Keep empty string as empty string to match urlparse behavior
        fragment = urlparse(resolved_url_str_with_frag).fragment
        url_info._original_fragment = fragment
        # Now remove fragment for further processing
        resolved_url_str = resolved_url_str_with_frag.split("#", 1)[0]

        logger.debug(
            f"URLInfo Resolved: resolved_url='{resolved_url_str}', fragment='{url_info._original_fragment}'"
        )
        ...
        if url_info._parsed.port is not None and not (
            0 <= url_info._parsed.port <= 65535
        ):
            url_info.is_valid = False
            ...
            logger.debug(
                f"URLInfo Invalid Port: raw_url='{url_info._raw_url}', error='{url_info.error_message}'"
            )
            # Set initialized before returning to avoid issues in finally block
            url_info._initialized = True
        ...
        logger.debug(
            f"URLInfo Parsed Resolved: parsed='{url_info._parsed}', original_trailing_slash={url_info._original_path_had_trailing_slash}"
        )

        # --- Pre-Normalization Validation ---
        # Validate the *resolved* parsed URL first for security issues
        logger.debug(
            f"URLInfo: About to call pre-normalization validate_url with parsed='{url_info._parsed}' for raw_url='{url_info._raw_url}'"
        )
        _is_valid_pre_norm, _initial_error_message = validate_url(
            url_info._parsed, raw_url=url_info._raw_url
        )  # Pass raw_url
        logger.debug(
            f"URLInfo Pre-Normalization Validation Result: is_valid={_is_valid_pre_norm}, error='{_initial_error_message}' for raw_url='{url_info._raw_url}'"
        )
        ...
        if _is_valid_pre_norm:
            ...
            try:
                ...
                logger.debug(
                    f"URLInfo Normalized: normalized_url='{url_info._normalized_url}'"
                )

                # --- Post-Normalization Validation ---
                # Re-validate the *normalized* URL to catch issues introduced by normalization
                logger.debug(
                    f"URLInfo: About to call post-normalization validate_url with normalized_parsed='{url_info._normalized_parsed}' for raw_url='{url_info._raw_url}'"
                )
                post_norm_valid, post_norm_error = validate_url(
                    url_info._normalized_parsed, raw_url=url_info.normalized_url
                )  # Pass normalized_url as raw_url context
                logger.debug(
                    f"URLInfo Post-Normalization Validation Result: is_valid={post_norm_valid}, error='{post_norm_error}' for raw_url='{url_info._raw_url}' (normalized: '{url_info.normalized_url}')"
                )
                if not post_norm_valid:
                    # If normalization created an invalid URL, mark as invalid
                    url_info.is_valid = False
                    url_info.error_message = (
                        f"Post-normalization validation failed: {post_norm_error}"
                    )
                    url_info._normalized_url = (
                        resolved_url_str  # Fallback to pre-norm resolved URL
                    )
                    logger.warning(
                        f"URLInfo Post-Normalization Invalid: raw_url='{url_info._raw_url}', normalized='{url_info._normalized_parsed}', error='{url_info.error_message}'"
                    )
                else:
                    ...
                    if url_info.base_url:
                        ...
                        if temp_base_url_info and temp_base_url_info.is_valid:
                            ...
                            if temp_base_url_info._normalized_parsed:
                                base_scheme = (
                                    temp_base_url_info._normalized_parsed.scheme
                                )
                                # Extract registered domain for the base URL
                                base_domain_parts = extract_domain_parts(
                                    temp_base_url_info._normalized_parsed.hostname
                                )
                                base_reg_domain = (
                                    base_domain_parts.get("registered_domain")
                                    if base_domain_parts
                                    else None
                                )
                                ...
                                base_reg_domain = None
                                base_is_valid = (
                                    False  # Mark as invalid if components missing
                                )
                                logger.warning(
                                    f"Base URL '{url_info.base_url}' was valid but missing normalized components."
                                )

                    # Extract components for the current URL from its normalized parsed result
                    current_scheme = (
                        url_info._normalized_parsed.scheme
                        if url_info._normalized_parsed
                        else None
                    )
                    current_netloc = (
                        url_info._normalized_parsed.netloc
                        if url_info._normalized_parsed
                        else None
                    )
                    current_hostname = (
                        url_info._normalized_parsed.hostname
                        if url_info._normalized_parsed
                        else None
                    )
                    current_domain_parts = extract_domain_parts(current_hostname)
                    current_reg_domain = (
                        current_domain_parts.get("registered_domain")
                        if current_domain_parts
                        else None
                    )
                    ...
                    logger.debug(
                        f"URLInfo Type Determined: url_type='{url_info.url_type}' for {url_info.raw_url} based on {url_info.base_url}"
                    )
                ...
                url_info._normalized_parsed = None
                logger.warning(
                    f"URLInfo Normalization Error: raw_url='{url_info._raw_url}', error='{url_info.error_message}'"
                )
            ...
            url_info._normalized_parsed = (
                url_info._parsed
            )  # Use the parsed version that failed validation
        ...
        url_info._normalized_url = (
            resolved_url_str if "resolved_url_str" in locals() else url_info._raw_url
        )
        logger.warning(
            f"URLInfo ValueError: raw_url='{url_info._raw_url}', error='{url_info.error_message}'"
        )
        ...
        url_info._normalized_url = url_info._raw_url  # Fallback
        logger.error(
            f"URLInfo Internal TypeError: raw_url='{url_info._raw_url}', error='{url_info.error_message}'",
            exc_info=True,
        )
        ...
        logger.error(
            f"URLInfo Unexpected Error: raw_url='{url_info._raw_url}', error='{url_info.error_message}'",
            exc_info=True,
        )  # Log with traceback

    finally:
        # Ensure initialization flag is set regardless of success or failure
        # Only set if not already set (to avoid AttributeError on immutable object)
        if not getattr(url_info, "_initialized", False):
            url_info._initialized = True
        logger.debug(
            f"URLInfo Init Complete: raw_url='{url_info._raw_url}', is_valid={url_info.is_valid}, norm_url='{url_info._normalized_url}', error='{url_info.error_message}'"
        )
