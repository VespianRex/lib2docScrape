...
def determine_url_type(
    ...
    """
    logger.debug(f"Determining type for '{raw_url}' against base '{base_raw_url}' (current_is_valid={current_url_is_valid})")
    ...
    if not base_url_is_valid:
        ...
        if not current_scheme and not current_netloc:
             ...
             logger.debug(f"Returning EXTERNAL (no valid base URL provided)")
             return URLType.EXTERNAL

    # --- Both current and base URLs are valid ---

    logger.debug(f"Comparing: current_scheme='{current_scheme}', base_scheme='{base_scheme}', current_reg_domain='{current_reg_domain}', base_reg_domain='{base_reg_domain}'")

    # Handle relative URLs: If scheme and netloc are missing, it's internal by definition (resolved against base)
    if not current_scheme and not current_netloc:
        logger.debug("CONDITION MET: Relative URL (no scheme/netloc). Returning INTERNAL.")
    ...
    if current_scheme == 'file' and base_scheme == 'file':
        logger.debug("CONDITION MET: Both current and base are file schemes. Returning INTERNAL.")
        return URLType.INTERNAL
    ...
    if not current_reg_domain or not base_reg_domain:
        logger.debug("Returning EXTERNAL (missing registered domain info for current or base URL, and not a file-to-file comparison)")
    ...
    domains_match = (current_reg_domain is not None and current_reg_domain == base_reg_domain)

    if schemes_match and domains_match:
        logger.debug("CONDITION MET: Schemes match: True, Registered Domains match: True. Returning INTERNAL.")
        return URLType.INTERNAL
    else:
        ...
        if not domains_match:
            reason.append(f"Domains mismatch ('{current_reg_domain}' vs '{base_reg_domain}')")
        logger.debug(f"CONDITION NOT MET: Schemes match: {schemes_match}, Domains match: {domains_match}. Returning EXTERNAL. Reason: {'; '.join(reason)}")
        return URLType.EXTERNAL