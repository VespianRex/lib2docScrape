...
def extract_domain_parts(hostname: Optional[str]) -> MappingProxyType[str, Optional[str]]:
    """
    ...
    if not h:
        ...
        return MappingProxyType({
            "subdomain": None, "domain": None, "suffix": None, "registered_domain": None
        })
    ...
    try:
        ...
        if not is_ip:
            if h == 'localhost':
                dom = 'localhost'
                reg_dom = 'localhost'
                logger.debug(f"extract_domain_parts: Hostname is 'localhost'.")
                ...
                try:
                    ...
                    logger.debug(f"extract_domain_parts: tldextract result for '{h}': {extracted}")
                except Exception as e:
                    logger.warning(f"extract_domain_parts: tldextract failed for hostname '{h}': {e}. Falling back to basic split.")
                ...
                logger.debug(f"extract_domain_parts: tldextract not available. Using basic split for '{h}'.")
                parts = h.split('.')
                if len(parts) >= 3:
                    sub = '.'.join(parts[:-2])
                    dom = parts[-2]
         ...
         logger.error(f"extract_domain_parts: Unexpected error parsing hostname '{h}': {e}", exc_info=True)
         # Return None for all parts on unexpected error
         sub, dom, suf, reg_dom = None, None, None, None

    result = {"subdomain": sub, "domain": dom, "suffix": suf, "registered_domain": reg_dom}
