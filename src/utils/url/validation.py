...
def validate_url(
    parsed: ParseResult, raw_url: str = None
...
def validate_scheme(p: ParseResult) -> tuple[bool, Optional[str]]:
    ...
    logger.debug(
        f"!!! validate_scheme: Checking scheme '{scheme}' against ALLOWED: {allowed} and DISALLOWED: {disallowed}"
    )
...
def validate_netloc(p: ParseResult) -> tuple[bool, Optional[str]]:
    ...
    if not hostname and scheme != "file":
        ...
        logger.debug(
            f"Netloc validation failed: Missing host for non-file scheme '{scheme}'."
        )
    ...
    if hostname:
        ...
        try:
            ...
            logger.debug(
                f"Checking IP hostname '{hostname}' against DISALLOWED: {disallowed_hosts}"
            )
            if hostname in disallowed_hosts:
                logger.debug(
                    f"Hostname '{hostname}' IS in disallowed list (SSRF risk). INVALID."
                )
            ...
            if ip.is_private:
                ...
                error_host = (
                    hostname if ip.version == 6 and ip.is_loopback else ip_check_host
                )
            ...
            if any(ord(c) > 127 for c in hostname):
                try:
                    ...
                    logger.debug(
                        f"Hostname '{hostname}' contains non-ASCII and failed IDNA encoding. INVALID."
                    )
            ...
            logger.debug(
                f"Hostname '{hostname}' is not a valid IP. Validating as domain."
            )

            # Check against disallowed hosts for SSRF
            logger.debug(
                f"Checking domain hostname '{hostname}' against DISALLOWED: {disallowed_hosts}"
            )
            if hostname in disallowed_hosts:
                logger.debug(
                    f"Hostname '{hostname}' IS in disallowed list (SSRF risk). INVALID."
            ...
            if "." not in hostname:
                if hostname == "localhost":
                    ...
                    logger.debug(
                        f"Hostname '{hostname}' is localhost and not disallowed. VALID."
                    )
                    return True, None
                else:
                    # Reject other single-label hostnames as per test expectation for 'invalid'
                    logger.debug(
                        f"Hostname '{hostname}' is single-label and not localhost. INVALID."
                    )
            ...
            if any(not label for label in labels):
                return (
                    False,
                    "Invalid domain label length or empty label",
                )  # Catches '..' case
            ...
            try:
                ...
                if any(c in URLSecurityConfig.CONFUSABLE_CHARS for c in temp_host):
                    logger.warning(
                        f"Potential homograph character detected in hostname: {temp_host}"
                    )
                ...
                if any(ord(c) > 127 for c in temp_host) and not any(
                    label.startswith("xn--") for label in encoded_host.split(".")
                ):
                    logger.warning(
                        f"Suspicious IDNA normalization for {temp_host} -> {encoded_host}"
                    )
...
def validate_port(p: ParseResult) -> tuple[bool, Optional[str]]:
    ...
    try:
        ...
        # Handle case where port is out of range and urlparse raises ValueError
        return False, f"Invalid port: {str(e)}"
...
def detect_path_traversal(path: str) -> bool:
    ...
    if URLSecurityConfig.PATH_TRAVERSAL_PATTERN.search(normalized_path_for_check):
        logger.debug(
            f"Path traversal attempt detected (robust pattern) in path: '{path}' (checked as: '{normalized_path_for_check}')"
        )
        return True
...
def detect_unc_path(p: ParseResult) -> bool:
    ...
    # Any path starting with // or \\ is considered a UNC path for security reasons
...
def validate_security_patterns(p: ParseResult) -> tuple[bool, Optional[str]]:
    ...
    full_decoded_url_part = (
        decoded_path + "?" + decoded_query if p.query else decoded_path
    )
    ...
    if any(c in URLSecurityConfig.CONFUSABLE_CHARS for c in full_decoded_url_part):
        logger.warning(
            f"Potential homograph character detected in URL: {full_decoded_url_part}"
        )
