from urllib.parse import urlunparse, parse_qsl, urlencode, urljoin
from .factory import create_url_info
from .info import URLInfo # Assuming URLInfo is still needed for type hinting or other reasons
...
class URLInfoManipulationMixin: # Renamed to avoid conflict if URLInfo is imported
    ...
    def join(self, path: str) -> 'URLInfo': # Renamed relative_url to path for consistency
        ...
        if not self.is_valid:
            ...
            return create_url_info(path, base_url=self.raw_url) # Attempt with raw_url as base
