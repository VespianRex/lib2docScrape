...
class URLSecurityConfig:
    ...
    INVALID_PATH_TRAVERSAL: Pattern = re.compile(
        r"(?:^|/)\.\.\.(\?|/|$)"
    )  # Detects /../ or ../
    ...
    SQLI_PATTERNS: Pattern = re.compile(
        r"'\s*OR\s*'|'\s*--|\s*UNION\s+SELECT", re.IGNORECASE
    )
    CMD_INJECTION_PATTERNS: Pattern = re.compile(r"[;`|&]")
    NULL_BYTE_PATTERN: Pattern = re.compile(r"%00|\x00")
    DOMAIN_LABEL_PATTERN: Pattern = re.compile(
...
class URLInfo:
        ...
        if hostname:
            ...
            try:
                ...
                try:
                    ...
                    if not all(
                        self.DOMAIN_LABEL_PATTERN.match(label) for label in labels
                    ):
        ...
        try:
            ...
            if hostname:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"IDNA encoding failed for {hostname}: {e}. Falling back."
                    )
                    netloc_norm = hostname.lower().rstrip(".")  # Fallback
                except Exception as e:
                    self.logger.error(
                        f"Unexpected error normalizing hostname {hostname}: {e}"
                    )
                    netloc_norm = hostname.lower().rstrip(".")  # Fallback

            # Add port if it's not the default for the scheme
            if port is not None and not (
                scheme in self.DEFAULT_PORTS and port == self.DEFAULT_PORTS[scheme]
            ):
                netloc_norm += f":{port}"
            ...
            if (
                self._parsed.netloc
                and not path_norm
                and not self._original_path_had_trailing_slash
                and temp_normalized_url.endswith("/")
            ):
    ...
    def _determine_url_type(
        ...
        try:
            ...
            if (
                norm_p.scheme.lower() == base_p.scheme.lower()
                and norm_p.netloc.lower() == base_p.netloc.lower()
            ):
    ...
    def query_params(self) -> dict[str, list[str]]:
        ...
        if (
            not self.is_valid
            or not self._normalized_parsed
            or not self._normalized_parsed.query
        ):
            return {}

        result: dict[str, list[str]] = {}
        for key, value in parse_qsl(
            self._normalized_parsed.query, keep_blank_values=True
        ):
            if key in result:
    ...
    def domain(self) -> str:
        ...
        return (
            self._normalized_parsed.hostname
            if self.is_valid
            and self._normalized_parsed
            and self._normalized_parsed.hostname
            else ""
        )
