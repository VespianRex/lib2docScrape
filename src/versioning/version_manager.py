...
class VersionManager:
    ...
    def _store_content(self, doc_id: str, version: str, content: str) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to store content for {doc_id} version {version}: {str(e)}")
    ...
    def _get_content(self, doc_id: str, version: str) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(f"Failed to get content for {doc_id} version {version}: {str(e)}")
            return None
    ...
    def add_version(
        ...
        for existing in doc_versions:
            if existing.content_hash == content_hash:
                logger.info(f"Content for {doc_id} already exists with version {existing.version}")
                return existing
    ...
    def compare_versions(
        ...
        if format == VersionDiffFormat.UNIFIED:
            ...
            diff_content = json.dumps({
                ...
                "changes": [
                    {
                        "type": "removed" if line.startswith("-") else "added" if line.startswith("+") else "context",
